...
keeping every result traceable to its source. Mechanical grammatical variants,
optional corpus co-occurrence, ranking, author periods, and result clusters are
deterministic retrieval aids.
...
- Query expansion is deterministic; mechanical grammatical variants are generated at query time.
- An external lexicon may be supplied explicitly for experiments; no bundled lexicon is required.
...
- **Quality** reports the strength of literal, structural, and
...
build. No other dataset download is required: the fonts, artwork, search code,
and optional generated search metadata are already part of the repository.
...
- **Startup resource error**: confirm the assets, manifest, and database
...
- `load_lexicon(...)`: load an optional caller-supplied query expansion file;
  with no path it returns an empty lexicon.
- `validate_advanced_criteria(...)`: validate advanced criteria.
- `configure_search_connection(...)`: configure SQLite search behavior.

Mechanical inflections and irregular grammatical variants are generated by
`common.py` at query time. No morphology configuration or bundled concept
lexicon is loaded during startup. Callers may pass an external lexicon to
`load_lexicon()` and the search functions when experimenting with explicit
